...
friendsRoutes.get("/",asyncHandler(async(req:any,res)=>{const{page,limit,skip}=parseQuery(req.query);const u=await User.findById(req.user!.userId);if(!u)throw new AppError(ERRORS.RESOURCE.USER_NOT_FOUND);const q:any={_id:{$in:u.friends}};if(req.query.search)q.username={$regex:req.query.search,$options:"i"};const[data,total]=await Promise.all([User.find(q).select("username displayName avatar country lastLoginAt").skip(skip).limit(limit),User.countDocuments(q)]);paginated(res,data,buildPage(page,limit,total),MSG.LIST("Friends"));}));
